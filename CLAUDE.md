...
**Current state (end of 2026-05-03 session, part 2):** **Step 4c shipped** — three commits across the substeps: `2070002` (4c-1 boot + WORKERFS validated), `ca870c2` (4c-2 typed FFmpegClient + COEP fix + drop dead `health-check.tsx`), `e876f09` (4c-3 + 4c-4 probe/extract/mux implemented + validated end-to-end on Three-Body MKV).  Earlier today: 4a (foundation, `fac632e`), architecture lock to Option B (`9e969c2`), 4b spike (`c8b14ee`).  All prior work (R1–R6a, R6b-presets/fonts, timing offsets, auto-alignment, R5 Hebrew/Arabic/Persian/Urdu, Step 3c desktop bundling for Linux) remains intact; CI green on Ubuntu / macOS / Windows for the test suite.  All step-4 work lands on `monorepo-restructure` branch (now ~50 commits ahead of `origin/main` from the whole restructure effort); `monorepo-restructure → main` merge is a separate decision when Connor wants to cut a release.

**🎯 Active focus: Step 4d — JS port of ASS generation + PGS rasterization.**  4c's video plumbing is done; the web app can probe/extract/mux real MKVs.  4d ports `loom_core/subs/processing.py::generate_ass_file` + `loom_core/rasterize/sup_writer.py` to TypeScript so subtitle outputs run entirely client-side.  PGS rasterization uses html2canvas per the 4b spike (visual equivalence, not byte-identical to desktop — 4d's sup_writer port operates on whatever pixels html2canvas produces).  ASS port is mechanical (string assembly + style/event manipulation), PGS port is heavy (PGS binary writer + epoch management + palette quantization + RLE encoding).
...
| **4d** | **🎯 next** | JS port of `loom_core/subs/processing.py::generate_ass_file` + `loom_core/rasterize/sup_writer.py`, operating on html2canvas pixel buffers | Subtitle outputs entirely client-side. |
