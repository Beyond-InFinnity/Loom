...
**Current state (2026-05-02):** R1–R6a + R6b-presets + R6b-fonts (library primitive) + timing offsets + auto-alignment complete on the `monorepo-restructure` branch. R5 closed out — Hebrew + Arabic + Persian + Urdu all shipped. **Step 3b complete (full feature parity with Streamlit)** — full backend (`loom_core` engine + `loom_api` FastAPI service) + Tauri desktop shell covering the full pipeline: file picker → video scan → dual-view style editor → timing offsets + auto-align → live preview → generate .ass/.sup → mux into MKV. CI pipeline is **fully cross-platform** (Ubuntu / macOS / Windows all green). **fontconfig drop landed 2026-04-26**, **Track A (font bundling) plumbing landed 2026-05-02** — the rasterizer's HTML template injects bundled `@font-face` blocks with explicit `unicode-range` routing; Chromium picks the correct Noto family per codepoint without fontconfig fallback.

**Active focus:** **Step 3c — Track B (Python bundling)** is the remaining 3c work. Track A still has one open follow-up: expanding from the 3-language staging set to the full Noto manifest.

**Track A — font bundling (✅ plumbing complete, font set still partial):**
- `scripts/fetch_noto_fonts.sh` pulls a 3-language staging set (NotoSans Latin, NotoSansJP, NotoSansHebrew — Regular + Bold only, ~10MB total) from official Noto release tags into `apps/desktop/src-tauri/resources/fonts/`. Idempotent; re-run is no-op if files present.
- `tauri.conf.json` declares `bundle.resources: ["resources/fonts/*"]`.
- `apps/desktop/src-tauri/src/lib.rs::spawn_sidecar` resolves `app.path().resource_dir() / "fonts"` and passes via `LOOM_FONT_DIR` env. Honors explicit env override (dev workflow). **Dev-mode caveat:** Tauri 2's `resource_dir()` in dev returns the build artifact dir, not `src-tauri/resources/`, so during `npm run tauri dev` you must set `LOOM_FONT_DIR=$PWD/apps/desktop/src-tauri/resources/fonts` manually. Production bundles read from the actual resource dir.
- `loom_core/fonts.py` now exposes `iter_faces()`, `face_weight()`, and a top-level `build_font_face_css(scanner) -> str`. The CSS emitter walks the scanner, deduplicates faces under their preferred (typographic) family name, coalesces each face's cmap into contiguous `unicode-range` descriptors (mandatory — emitting 30k literals per CJK face would balloon CSS to MBs), and emits one `@font-face` block per face with `font-family`, `font-weight` (raw `OS/2.usWeightClass`), `src: file://… format(…)`, `unicode-range`.
- `loom_core/rasterize/pgs.py::_build_fullframe_html` injects the @font-face CSS at the top of `<style>`. Tested end-to-end on a Latin+Hebrew+Japanese frame: zero tofu, Hebrew chars correctly route from a NotoSansJP-Bold request to NotoSansHebrew-Bold via unicode-range. 22 new tests in `tests/test_font_validation.py`.
- **Open Track A follow-up:** expand the staging set to the full design manifest (Sans CJK SC/TC/JP/KR, Sans Thai, Naskh Arabic, Nastaliq Urdu, Sans Devanagari/Bengali/Tamil/Telugu/Gujarati/Gurmukhi, Sans for Latin/Cyrillic/Greek). Mechanically just adding more `fetch_pair` calls to `scripts/fetch_noto_fonts.sh`; CSS generator already handles arbitrary face count. Total bundle will land ~150–250 MB depending on how the per-region CJK split shakes out.

**Track B — Python bundling (next concrete step):** Stand up uv-standalone with python-build-standalone in the Tauri build pipeline. Pick a python-build-standalone tarball (CPython 3.10/3.11, libc/macOS/Windows variants), pin it. Build a uv-managed venv against that interpreter containing the `loom_api` + `loom_core` dependencies (most-difficult: Playwright, fugashi, aksharamukha → lxml). Drop the result under `src-tauri/resources/python/`. Update the Rust sidecar spawn to point uvicorn at the bundled interpreter + venv instead of the developer's `LOOM_UVICORN`. PyInstaller one-folder is the fallback if uv-standalone hits sharp edges with the heavy deps.

Still open across both tracks: Tauri auto-updater compat with multi-hundred-MB resource directories; python-build-standalone interpreter pin; unidic-lite 250MB shrinking question.
