...
**🎯 Active focus next session — NETFLIX PORT (now the active workstream; store review is in flight and out of our hands):**
1. **Netflix support — the new build target.** Verdict was GO (`NETFLIX_RECON.md` + `memory/reference_netflix_caption_acquisition_2026.md`). Key facts to re-load before coding: Netflix's timedtext manifest is **MSL-encrypted**, so caption acquisition is **NOT** a webRequest interception like YouTube — it's a **MAIN-world `JSON.parse` hook** on the player's own manifest fetch (player API + DOM access confirmed in recon). **The one hard constraint: image-based (bitmap) CJK subtitles** — some Netflix tracks are pre-rendered images, not text, so they can't be tokenized/romanized without OCR. Sequence: (a) run `spike/netflix/capture-kit.js` on a live authenticated session to capture real manifest + VTT/TTML payloads; (b) seed/validate the parsers in `spike/netflix/{parse-vtt,parse-ttml,parse-test}.mjs` against that real data; (c) build the platform-agnostic adapter (~1180 LOC est. per recon) so the existing overlay/annotate/romanize pipeline is fed by a Netflix source as well as YouTube.
