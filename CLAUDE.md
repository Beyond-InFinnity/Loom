...
| 5h-3 | ✅ LIVE-VERIFIED (JJK + Crouching Tiger, this session) | `entrypoints/netflix.content.tsx` (ISO overlay mount) + overlay seam on `CaptionPlatform` (`playerRootSelector` + `videoSelector` + `hideNativeCaptions`/`restoreNativeCaptions`, generalizing `player-scale.ts` + `stream.ts` + `caption-context.tsx`) + `lib/overlay/netflix-player-anchor.ts`. **Anchor is `div[data-uia="player"]`** (the LCA of the `<video>` and Netflix's control chrome, found via DOM probe) — NOT `video-canvas`, which trapped the overlay one stacking-context below Netflix's controls so the pill was unclickable while the chrome was up. Playhead reuses `<video>.timeupdate`. **Post-first-run UX fixes (this session):** pill/panel repositioned off Netflix's top-right report-flag via `lib/overlay/pill-position.ts` (platform-aware, YT unchanged); clicks on the pill/panel `stopPropagation` so they no longer drive Netflix play/pause + dblclick-fullscreen (`lib/overlay/stop-player-events.ts`; safe with the capture-phase dismiss listener). Furigana/romanize/presets all confirmed against the **prod Railway API** (dev build via `LOOM_API_BASE`). | First Netflix overlay render; full parity with YouTube. **Verified end-to-end on real authenticated tabs.** |
| 5h-4 | ✅ done (this session) | Per-track **stable `id`** (YouTube vssId, Netflix manifest trackId) added to `CaptionTrack` + both serializers — fixes same-language duplicates (plain "English" vs "English (CC)") that previously collapsed on `languageCode`: dual-highlight in the picker, duplicate React keys, and an events-cache collision. `auto-pick` now ranks `manual > asr` then **standard `subtitles` > SDH `closedcaptions`** (via `isCc` flag = Netflix `rawTrackType==="closedcaptions"`); falls back to CC when CC-only (JP-anime / Thai origin); CC stays user-selectable. `forced`+`none` already excluded MAIN-side in 5h-2. +3 vitest. _(Per-language variant tails `;0;0;0/3/4/13;` are now distinct tracks via id; no further handling needed.)_ | Correct track selection on real Netflix tracklists. |
