...
**Current state (end of 2026-06-01 session):** Steps 1–4 shipped (4f live; 4g Streamlit-deletion pending). Step 5 (browser extension) is the active frontier with **5a–5f shipped — only 5g (store distribution) remains**. The extension renders the full 4-layer stack on YouTube: native (Bottom) + foreign (Top) + per-token annotation ruby (furigana / Pinyin / Zhuyin / Jyutping, auto-routed via `classifyLang` / Korean RR) + a **secondary full-utterance romanization line (5e — shipped)** that covers every phonetic-layer language (CJK + Korean get both ruby and the line; Cyrillic / Thai / Indic / Hebrew / Arabic-Persian-Urdu get the line as their whole phonetic surface). Both layers fetch as a single batch POST on activation — `/annotate/batch` per (target_track, phonetic_system) and `/romanize/batch` per (track, phonetic_system, long_vowel_mode), ~3–4 s, then quiet. Per-tab activation: every YouTube tab starts dormant with a "Loom" pill; click to activate; `sessionStorage` persists across same-tab reloads. The settings panel covers live track switching (manual/ASR badges + processing-tier indicators), per-layer `tlang` overrides, a 4-slot position picker (top/bottom × 1/2 with auto-swap), full per-layer styling (color + font + size + alpha + outline color/alpha + glow radius/color/alpha for Bottom/Top/Annotation/Romanization), annotation toggle + phonetic-system override, romanization toggle + long-vowel mode, alternate-orthography ruby (zh-Hant → Simplified) with Distinct/Merged tier highlights, 28 lang-aware color presets (from `/styles/presets`), and an inline HSV color wheel (react-colorful) — functionally on par with the desktop Style editor. All persisted to `browser.storage.local`. **Extension is at v0.1.3, deployed** — the 0.1.1 `kind=asr` experiment regressed auto-translate and was reverted (0.1.2); Chinese / ASR-track caption rendering is **healthy in 0.1.3** (verbose `[Loom …]` logging is temporarily always-on in production — re-gate behind `IS_DEV` before public launch). **Build-time dev/prod split shipped** (PUBLISH_PLAN step 1): separate output dirs, distinct extension IDs + names ("Loom" vs "Loom (Dev)"), API base + verbosity injected via `lib/env.ts`. **Netflix port: recon complete, verdict GO, deferred to post-YouTube-launch** (see `NETFLIX_RECON.md` + `spike/netflix/`). Production: `api.loom.nerv-analytic.ai` serves `/annotate/batch` + `/romanize/batch` + `/styles/presets`; web app live at `https://loom.nerv-analytic.ai` (now with `/privacy`). Tests: **626** `test_` functions in `loom_core` (`tests/`, +17 from `/romanize/batch`) + 7 vitest spec files (~108 cases) in `apps/extension`. _(static counts — pytest/vitest not runnable in this session's env; re-run to confirm pass totals.)_

**🎯 Active focus next session:**
1. **5g — store distribution (the headline gate to public launch).** Sequence: capture 5 live screenshots at 1920×1080 (per `STORE_LISTING.md` — headline JA, settings panel, zh alt-orthography, non-CJK romanization, color presets) → sign for Firefox AMO self-distribution (`apps/extension/SIGNING.md` Path A: `npm run zip` → AMO web uploader → signed `.xpi`) → Chrome Web Store unlisted ($5 + review). Listing copy + permission justifications are drafted and ready in `STORE_LISTING.md`.
2. **Re-gate `logDev` behind `IS_DEV` before public launch.** `lib/env.ts::logDev` is currently always-on in production (added in 0.1.3 to diagnose the now-resolved caption bug). Restore the `if (IS_DEV)` gate before listing.
3. **Owed follow-ups (carry over from 5b–5e):**
...
5. **Netflix port (post-launch).** Owed prerequisite: run `spike/netflix/capture-kit.js` on a live authenticated session to seed the VTT/TTML parsers with real data, then build the platform-agnostic adapter (~1180 LOC est. per recon).
...
| 5g | 🟡 store-prep underway | **Build-time dev/prod split ✅** (`4f96296` — PUBLISH_PLAN step 1: separate output dirs/IDs/names, API base + verbosity via `lib/env.ts`). **Store kit ✅** (`525c46b` + `e82bc7c` — privacy page, version footer, brand icons via `scripts/gen-icons.py`, AMO `data_collection_permissions: websiteContent`, `STORE_LISTING.md` + `SIGNING.md`). **Remaining:** capture 5 screenshots → AMO self-distribution signing → Chrome Web Store; re-gate `logDev` behind `IS_DEV`; Chrome MV3 verification; tlang=en parser anomaly; stale-URL fix. | Ship-readiness polish before public announcement.  See PUBLISH_PLAN.md for the 6-step rollout. |
...
| 5 | 🟡 5a–5f ✅; 5g store-prep underway | WXT browser extension at `apps/extension/`. 5a-5c shipped 2026-05-20/21 (`2bc507c`, `b1d2a82`); 5d shipped 2026-05-22 (`/annotate/batch` single-shot, `f7f4d66`); 5f settings UI 2026-05-22/23; **5e secondary romanization line shipped 2026-05-26** (`b35451d` + `fa760f3`, `/romanize/batch`). 5g (store distribution) underway: dev/prod split + store kit shipped 2026-05-30, screenshots + AMO signing remaining — see Quick-Start Step 5 substeps table + `PUBLISH_PLAN.md`. Extension at **v0.1.3** (kind=asr regression reverted in 0.1.2; caption rendering healthy). YouTube caption access uses webRequest interception + lang-swap (PO-token moat documented in `memory/reference_youtube_caption_acquisition_2026.md`); **Netflix path: recon complete, verdict GO, deferred to post-launch** (`NETFLIX_RECON.md`). Major OCR data source from 5f's `opt_in_training` toggle wire-up (still owed). |
