...
**Current state (end of 2026-05-22 session):** **5a → 5d all shipped.**  Extension renders dual subs on YouTube **with per-character annotation ruby** — furigana for Japanese, Pinyin / Zhuyin / Jyutping for Chinese variants (auto-routed via classifyLang's chineseVariant), Revised Romanization for Korean.  Annotation pipeline locked at a single `/annotate/batch` POST per (target_track, phonetic_system) on activation, ~3-4 sec wait then permanent quiet (production verified for `zh-Hans` → Pinyin).  **Per-tab activation:** every YouTube tab starts DORMANT with a tiny "Loom" pill; user clicks to activate the full pipeline; sessionStorage persists across same-tab reloads.  **Settings panel substantially complete** (this is what was queued as "5f" — landed organically as 5d-diagnostic): live track switching with manual/ASR badges + processing-tier indicators, per-layer tlang overrides, position picker (top/bottom × slot 1/2 with auto-swap), per-layer styles (color + font family + font size for Bottom/Top/Annotation, mirroring desktop's StyleConfig), annotation toggle + phonetic-system override per layer, native-language base preference.  All persisted to `browser.storage.local`.  Production live: `api.loom.nerv-analytic.ai` now serves `/annotate/batch` (deployed via fast-forward of `main` from `monorepo-restructure` — Railway only tracks `main`).  Step 4f web deploy still live at `https://loom.nerv-analytic.ai`.  Tests: **609 passing** in `loom_core` (pytest) + **164 passing in `apps/extension`** (vitest across lang-code / lang-support / auto-pick / url-picker / orthography-tables / build-segments).

**Late-session add (2026-05-22):** **Alternate-orthography under-ruby layer shipped** — Traditional Chinese characters now render their Simplified form as a second `<rt>` BELOW the base glyph (independent of, and composable with, the existing Pinyin/Zhuyin/Jyutping over-ruby).  Built in two commits:
- **Pass 1 (`88d0a24`):** new shared workspace `packages/orthography-tables/` (mirrors `packages/api-client/` shape) with a builder that reads OpenCC's `TSCharacters.txt` and emits a 4105-entry sorted-key JSON.  Each entry is `{to, collapse}` — `to` is the canonical simplified form; `collapse` lists OTHER trad chars that forward-map to the same simplified glyph (the pedagogically significant "identity-hidden" cases like 髮/發 → 发).  Generic, orthography-neutral `resolveOrthographyVariants(langCode)` resolver — today only Traditional Chinese (zh-Hant, zh-TW, zh-HK, zh-MO, yue, yue-Hant) resolves; future variants (Japanese kana, Serbian/Kazakh Cyrl↔Latn) slot in as one new descriptor + one new rule.  Reverse direction (simp→trad) is lossy + explicitly out of scope.  17 vitest cases.
- **Pass 2 (this commit):** rendering + UI.  Reuses the existing ruby path — new `RichSegment[]` data shape merges annotate spans + orthography lookup, walked by a refactored `AnnotatedText` that emits double-sided `<ruby>` (over-rt for phonetic reading, under-rt for variant form).  Three-tier base-char highlight: absent (no styling) / `clean` (1:1 mapping, default cyan) / `collapse` (forward-collapse merge, default amber).  Vertical gap on under-rt via `transform: translateY()` (NOT margin — Chromium ruby layout bug; matches the existing annotation_gap convention).  Settings-panel section appears only when the active track resolves a variant (data-driven gate; mirrors how the existing AnnotateRow gates on language).  Per-layer enable (target / native) + shared highlight toggle + shared colors.  All persisted to `browser.storage.local`.  12 additional vitest cases for the segment-builder.
...
**Pending state (next session check):** working tree clean.  3 unpushed commits on `monorepo-restructure` since last `origin/main` fast-forward: `f7f4d66` (batch endpoint + frontend rewrite — already in `main`/Railway), `b31a725` (slot pinning — local only), `aee664b` (per-layer styles — local only).  Push to `main` before next deploy.
...
| 5f | 🟡 substantially shipped this session | `components/settings-panel.tsx` + `components/{loom-app,dormant-pill}.tsx` + extended `caption-context.tsx`.  Per-tab activation, live track switching, per-layer tlang, position picker, per-layer styles (color + font + size), annotation toggle + phonetic-system picker, native lang preference.  **Pending:** `opt_in_training` flag wire-up (lands with step 6's OCR pipeline; no archival code yet). | User-controllable demo surface.  Owner-key bypass already on the slim API via `X-Loom-Auth`. |
...
| Alternate-orthography under-ruby (zh-Hant → Simplified) | ✅ table | — static client-side lookup | — | ⏳ | ✅ |
| Forward-collapse highlight tier (identity-hidden chars) | ✅ data | — | — | ⏳ | ✅ |
