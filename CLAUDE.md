...
**5g 🟡 in submission (public launch):** dev/prod split (`4f96296`); store kit (`525c46b`+`e82bc7c`: privacy page, icons, AMO `data_collection: websiteContent`, `STORE_LISTING.md`+`SIGNING.md`); global on/off kill switch + dev-only owner key (`lib/enabled.ts`); `/support`+`/donate` pages; bumped to **v0.1.7** (0.1.6 was a self-distributed AMO test; AMO versions unique across channels). Field copy in `SUBMISSION_0.1.7.md`; plan in `PUBLISH_PLAN.md`. **In flight:** AMO listed + Chrome Web Store review. **After approval:** Chrome MV3 runtime verify, tlang=en parser, stale-URL fix.
...
- **5h-5** 🔲 DEFERRED — settings-panel platform awareness (hide `tlang=` when `!supportsTranslate`; no manual/ASR badge on Netflix) + "image-only on this title" degradation. Folds into the UI overhaul.
...
| 5 | 🟡 5a–5f ✅; 5g in submission; 5h (Netflix) first pass ✅ | WXT browser extension at `apps/extension/`. 5a–5f shipped May 2026 (`2bc507c`→`fa760f3`). 5g (store distribution) **in submission** at **v0.1.7** (AMO listed + Chrome Web Store) — see Quick-Start + `PUBLISH_PLAN.md` + `SUBMISSION_0.1.7.md`. YouTube caption access uses webRequest interception + lang-swap (PO-token moat in `memory/reference_youtube_caption_acquisition_2026.md`); **Netflix port first pass ✅ live-verified 2026-06-19 (5h-1→5h-4)** — MAIN-world JSON.parse manifest hook + `div[data-uia="player"]` overlay anchor (see Quick-Start Step 5h + `NETFLIX_RECON.md` + `memory/reference_netflix_caption_acquisition_2026.md`). **Next workstream: UI/defaults overhaul** (consistency + accuracy of per-language controls + defaults). Major OCR data source from 5f's `opt_in_training` toggle wire-up (still owed). |
