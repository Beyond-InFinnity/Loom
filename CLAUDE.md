...
| R5-4 phase (b): RTL rendering (ASS + PGS + preview, Hebrew pilot) | 🔲 |
...
- R5-4 phase (b): RTL rendering with Hebrew as the pilot. Requires RTL-aware output in `.ass` (alignment + potential `\frx`), PGS (CSS `direction: rtl; unicode-bidi: isolate`), and preview iframe. Hebrew chosen over Arabic because: no contextual shaping, no cursive joining, bidi cooperation is extremely well-tested in browser stacks. Once RTL plumbing is in place, Arabic/Persian/Urdu block romanization is the remaining scope.
