...
**Current state (end of 2026-05-08 session):** **Steps 4d + 4e fully shipped.  Step 4f deployed live.**  The web app is reachable at `https://loom.nerv-analytic.ai` (Vercel, custom domain), the slim API at `https://api.loom.nerv-analytic.ai` (Railway, custom domain via the auto-generated `loom-production-975d.up.railway.app`).  Owner bypass auth is live (Tier A — see Owner Auth Roadmap).  All step-4 code landed on `monorepo-restructure` then fast-forward-pushed to `main` at `229b2f7`; `monorepo-restructure` is one docs commit ahead at `e160a5c` (push to `main` deferred until next code change since docs-only would re-deploy Vercel for nothing).  Test suite: **609 passing** across 19 files (`loom_core` only; `loom_api` smoke-tested via live cURL).
...
- **Docs consolidation** (`e160a5c`): Quick-Start + Capability Matrix refreshed for the shipped state; "Roadmap Beyond Step 4" section added with v1.5 follow-ups (PGS preserved-event rendering, /annotate client wiring, concurrent-event merging, SRT crawl-bait skip, speaker-tag toggle, HI-track auto-warning), v2 UX expansion (per-layer style editor + presets + offsets + auto-align + live preview wiring), long-term architecture (Web Worker rasterization, Canvas2D direct draw, observability), and 4f/4g closing items.  Owner Auth Roadmap section captures Tier A (live), Tier B (Google OAuth, deferred to step 5+), Tier C (Cloudflare Access, indefinitely deferred).

**🎯 Active focus next session:**
1. **4f production verification (the biggest owed item).**  Drop a real episode at `loom.nerv-analytic.ai/generate`, watch end-to-end completion, download `.ass` + `.sup`, mux back into source, play in mpv, confirm subs render at correct times + positions.  This is the canonical "deploy actually works" gate — pre-deploy fixes (`1af8a9d` fonts, `e6f2be7` perf, `e340380` + `96e6148` classifier) all individually verified but the post-fix end-to-end mpv playback test is still owed.
2. **4g Streamlit deletion** (gated on 4f passing): `rm loom_app.py` + `rm -rf app/` + drop `streamlit, pandas, pyarrow, pydeck, altair` from `requirements.txt` + update CLAUDE.md Project Structure section.  Verify `apps/desktop` uvicorn sidecar still boots after the dep prune.
3. **v1.5 picks** — start whichever surfaces first as a felt need.  Cheapest to ship is the SRT crawl-bait skip (~10 LOC, fixes "For best IPTV provider..." spam pollution).  Highest user-visible-impact is PGS preserved-event rendering (~300 LOC, fixes signs/karaoke missing from `.sup` output).  Both fully scoped in the "Roadmap Beyond Step 4 → v1.5" section.

**Pending state (next session check):** working tree clean.  `monorepo-restructure` is at `e160a5c` (one docs commit ahead of `main`); the next code commit on this branch should fast-forward `main` together with itself to keep Vercel + main in sync.  No uncommitted local changes.  Survey scripts (`/tmp/loom-survey.mjs`, `/tmp/loom-survey-mkv.mjs`, `/tmp/verify-frieren-raw.mjs`) are throwaway harnesses from the multi-style classifier validation — feel free to delete or rerun.
