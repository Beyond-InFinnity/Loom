...
**Current state (end of 2026-05-20 session):** **Steps 5a + 5b fully shipped.**  WXT browser extension foundation is on disk (Firefox MV2 verified, Chrome MV3 untested), and YouTube caption acquisition works end-to-end via the webRequest + lang-swap architecture — dual subs render in real time on YT watch pages, synced to playback.  Step 4f web deploy still live at `https://loom.nerv-analytic.ai` + `https://api.loom.nerv-analytic.ai` (Tier-A owner bypass auth, slowapi rate-limit, CORS now includes `chrome-extension://.*` + `moz-extension://.*` regex for the extension origin).  All step-5 code committed at `2bc507c`; `monorepo-restructure` is **2 commits ahead of `origin/monorepo-restructure`** (the previous docs commit `52c09b1` + the new 5a/5b commit `2bc507c`).  Test suite unchanged: **609 passing** across 19 files (`loom_core` only; extension is runtime-verified, no automated tests yet).

**Highlights this session (2026-05-19 → 2026-05-20):**
...
- **Architectural pattern that survives:** MAIN-world content script reads `#movie_player.getPlayerResponse()` for the tracklist metadata + programmatically clicks `.ytp-subtitles-button` to trigger YT's own tokenized timedtext fetch.  Background's `webRequest.onBeforeRequest` captures the pot-bearing URL.  ISOLATED content script polls background for the URL, runs the fan-out via the portable `lib/captions/fanout.ts` module, parses json3, feeds into `CaptionStream`.  React provider + status-aware pill display the active target + native caption text in real time.
...
1. **5c — dual-subs overlay (~200-300 LOC).**  Replace the top-right pill with a real overlay positioned above YouTube's caption area.  Hide YT's own caption box.  Handle fullscreen transitions.  Bottom (native) + Top (target) typography matched to the desktop's `_build_fullframe_html` styling.  No romanization yet — that lands in 5d.  This is the visual upgrade that turns the current 5b smoke-pill ("captions appear in a top-right debug bubble") into "captions appear where the user expects them, looking like a real product."
2. **5d — wire `/romanize` from `@loom/api-client`.**  Take each target-language event's text, fan it through the slim API with caching, render the romanized string as a third overlay layer above the target text.  This is when the demo finally becomes the headline ("dual subs PLUS per-character readings").  The api-client middleware path (X-Loom-Auth) is already proven from 5a's popup smoke.
3. **Owed follow-ups from 5b** (in priority order):
   - **Page-reload stale-URL.**  On YT page reload where CC was previously enabled (`aria-pressed=true` on initial check), the OFF→ON→OFF triple-click sometimes doesn't trigger a fresh YT fetch — background returns a stale URL from the previous session and fan-out returns empty bodies.  Recovery is "reload the extension" which works but is bad UX.  Permanent fix likely: invalidate `background.captured` map entry on `yt-navigate-finish` and re-trigger CC after the next message round-trip.
   - **tlang=en parser anomaly.**  Lang-swap returns full body (~64 kB) but the json3 parser extracts only 1 event from tlang responses.  Doesn't bite on videos with both native target AND native English (like our test video), but will fail on JA-only videos that need machine translation for Bottom.  Likely: tlang responses use word-level segs or a different events structure than native tracks.  Investigate before relying on tlang for production Bottom.
   - **Chrome MV3 verification.**  Whole 5b was developed and tested on Firefox MV2 only.  WXT builds both targets; need to load-unpacked in Chrome and re-verify the full flow.  `world: "MAIN"` is MV3-native in Chrome so should "just work."
4. **5e/5f/5g** queued per the Step 5 substeps table below.  5f (settings UI) is where the user-toggleable phonetic-system selector (Pinyin vs Zhuyin for both zh-Hans + zh-Hant per Connor's earlier framing) lands.

**Pending state (next session check):** working tree clean.  `monorepo-restructure` is 2 commits ahead of origin (`52c09b1` docs + `2bc507c` 5a/5b code).  No uncommitted changes.  Spike harness file (`apps/extension/entrypoints/spike.content.ts`) was removed in the final commit but lives in `2bc507c`'s ancestor history — resurrect from git if YT changes ever break the architecture and we need to re-verify the lang-swap session-binding assumption.
...
| 5b | ✅ `2bc507c` | `entrypoints/yt-main.content.ts` (MAIN world) + `entrypoints/background.ts` (webRequest) + `lib/captions/{discover,fanout,stream,auto-pick,types}.ts` + `components/caption-context.tsx` + status-aware `loom-pill.tsx` | YouTube caption acquisition via webRequest interception + lang-swap.  Pill displays live target + native caption text synced to playhead.  Verified Firefox MV2. |
| 5c | 🔲 | Real overlay component positioned above YT's caption area; fullscreen-aware; dual-layer typography; YT's own caption box hidden | Visual upgrade — captions appear where the user expects them, not in a debug pill. |
| 5d | 🔲 | `/romanize` fan-out via `@loom/api-client` with caching; Romanized layer rendered above Top in the overlay | The headline — per-character readings appear live above the foreign text. |
| 5e | 🔲 | `/annotate` for furigana / per-character ruby; CJK-specific rendering path | Polish: the visually distinctive layer that makes Japanese feel readable to learners. |
