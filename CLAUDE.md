...
**🎯 Active focus — NEXT PHASE: UI INTERNATIONALIZATION (i18n).** v0.3.0 SHIPPED (4 platforms: YouTube, Netflix, iQIYI, WeTV; pushed to `main` + uploaded to AMO + Chrome). The settings panel + popup are currently **English-only**; a real Japanese user (learning Chinese on iQIYI, found via AMO review) flagged the lack of a localized UI as a real barrier. **Next focus: expand the extension UI to support other languages, starting with Japanese** (then likely Chinese / Korean — the learner audiences). Scope = the popup + settings-panel strings + platform hints (the subtitle OUTPUT already works in every language; this is purely the chrome/UI text). No framework yet — pick a light i18n approach (e.g. a string table keyed by `browser.i18n.getUILanguage()` / a small dictionary module) that fits the shadow-root React UI.
