...
**Current state (end of 2026-05-03 session):** Step 4 underway. **4a (foundation) + 4b (PGS-in-browser spike) both shipped today** — commits `fac632e` (4a workspace + api-client + web scaffold), `9e969c2` (architecture lock to Option B), `c8b14ee` (4b spike + verdict).  All prior work (R1–R6a, R6b-presets/fonts, timing offsets, auto-alignment, R5 Hebrew/Arabic/Persian/Urdu, Step 3c desktop bundling for Linux) remains intact; CI green on Ubuntu / macOS / Windows for the test suite.  All step-4 work lands on `monorepo-restructure` branch (which is ~47 commits ahead of `origin/main` from the whole restructure effort); `monorepo-restructure → main` merge is a separate decision when Connor wants to cut a release.

**🎯 Active focus: Step 4c — ffmpeg.wasm wiring.**  Architecture is locked, foundation + spike are done.  Next concrete work: integrate `@ffmpeg/ffmpeg` into `apps/web/`, set it up to run in a web worker so the main thread stays responsive, expose three operations (probe metadata + track list, extract a subtitle track to bytes, mux generated .ass / .sup back into the source container).  The web worker boundary matters for UX — multi-GB ffmpeg.wasm operations would freeze the UI without it.  No further spike work needed before 4c; html2canvas was the architecture-gating unknown and it survived.
...
| **4c** | **🎯 next** | ffmpeg.wasm wiring: probe / extract / mux in a web worker | Video plumbing client-side. |
| 4d | 🔲 | JS port of `loom_core/subs/processing.py::generate_ass_file` + `loom_core/rasterize/sup_writer.py`, operating on html2canvas pixel buffers | Subtitle outputs entirely client-side. |
