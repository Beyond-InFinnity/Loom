...
**Recent (2026-04-20):** (0) R6b-fonts — `loom_core/fonts.py::validate_font()` library primitive over fc-match + fontTools cmap. Checks availability (fontconfig silently falling back to Liberation/DejaVu) AND glyph coverage against a per-language representative char sample. Regression test asserts every romanization language has a sample registered. UI integration deferred. (1) R5-1 katakana-as-furigana — `INLINE_FURIGANA_RE` widened to `[ぁ-んァ-ヶー]+` so tier-1 author glosses like 重力(グラビティ) / 本気(マジ) / 宇宙(スペース) flow through `_kana_to_romaji` correctly and preserve katakana in the annotation layer (author's semantic choice for loanword/slang readings). (2) Universal romanization polish — `_polish_romaji(text, *, capitalize)` at the tail of every factory: fullwidth CJK → Latin punctuation, strip space before close-punct, opt-in sentence-initial caps for source-case-less scripts (ja/zh/yue/ko/Indic). Fixes long-standing "space before 。" and all-lowercase romaji; pinyin's CJK-punct-drop bug (sentence boundaries silently lost) also fixed. (3) R5-2 Indic block romanization — Hindi/Bengali/Tamil/Telugu/Gujarati/Punjabi via `aksharamukha` (preferred over sanscript because the latter produces phonologically distorted Tamil). (4) R5-3 Devanagari per-akshara annotation — `_split_devanagari_aksharas` classifies chars by Unicode range into starter/extender/halant and groups consonant clusters + attached matras into single aksharas; each akshara gets its IAST reading via aksharamukha. Hindi only for now; algorithm generalizes across Brahmic scripts. (5) Zhuyin rendering fix, Thai IPA engine swap, `/generate/suggest-filename` + audio-default selector in mux advanced (all earlier in day).
...
**Devanagari per-akshara annotation (R5-3):** Hindi only. `_split_devanagari_aksharas(text)` returns `list[(segment, is_akshara)]` where aksharas are consonant clusters (one or more consonants joined by virama U+094D) plus attached matra/modifiers. Character classes via Unicode ranges: starters (U+0904-U+0914 vowels, U+0915-U+0939 consonants, U+0950 OM, U+0958-U+095F + U+0978-U+097F extended consonants, U+0960-U+0961 + U+0972-U+0977 extended vowels); extenders (U+0900-U+0903 signs, U+093A-U+093D nukta+avagraha, U+093E-U+094C + U+094E-U+094F matras, U+0951-U+0957 accents, U+0962-U+0963 vocalic matras); virama U+094D joins the next starter as a conjunct consonant. `_make_devanagari_annotation_func()` runs aksharamukha on each akshara individually; non-Devanagari chars pass through as `(char, None)` spans. Algorithm generalizes across Brahmic scripts — bn/ta/te/gu/pa can be wired analogously once needed.
...
| R5-3: Hindi/Devanagari per-akshara annotation | ✅ |
...
- Extending per-akshara annotation to bn/ta/te/gu/pa is a one-line wiring in `get_annotation_func` per script once a per-script splitter exists — the Devanagari algorithm (starter consonant/vowel + virama-joined conjuncts + extender matras/signs) generalizes cleanly across Brahmic scripts. Deferred behind R5-4 + R5 polish until a user need surfaces.
