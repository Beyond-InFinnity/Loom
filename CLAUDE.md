...
| CI ph 1+2 | ✅ | GitHub Actions matrix on push to main/monorepo-restructure + PRs to main. Phase 1 Ubuntu green, phase 2 macOS green. Includes pytest + Playwright Chromium rasterize + font-validator self-check. |
| CI ph 3 | 🔲 | Add `windows-latest` to `matrix.os`. System deps step pre-scaffolded (choco install ffmpeg/tesseract; intentionally NO fontconfig install to exercise `loom_core/fonts.py`'s graceful-degradation path). |
