...
- R5-4: Arabic/Persian/Urdu + Hebrew — RTL pilot (Hebrew first since it has no contextual shaping, no cursive joining, and bidi cooperation is extremely well-tested in browser stacks). Requires RTL-aware rendering in both `.ass` (alignment + potential `\frx`) and PGS (CSS `direction: rtl; unicode-bidi: isolate`). Font bundling/availability already handled for ar/fa/ur/he via `_SCRIPT_FONTS`.
