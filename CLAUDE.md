...
**🎯 Active focus — NEXT PHASE: expand Loom to a 3rd streaming platform (beyond YouTube + Netflix). v0.2.2 shipped (committed + pushed); the only remaining 0.2.2 task is the store upload (Connor's).**

**Next phase — platform expansion.** Connor wants to add another platform; candidates floated: Crunchyroll, HBO Max (Max), WeTV, iQIYI (+ open to others), and is interested in a plan to one-shot as many as feasible. The architecture is READY for this: the `CaptionPlatform` seam (`lib/captions/platform/`) + the shared pipeline (overlay / annotate / romanize / settings / playhead / the 0.2.2 SPA `autoMount` lifecycle) carry over unchanged. A new platform ≈ one `CaptionPlatform` impl + one MAIN/content entrypoint per site, differing mainly in (a) how subtitle TEXT is acquired and (b) player / video / anchor selectors.
