...
**Active focus:** **Step 3c (bundling for distribution)**. Architectural decisions resolved 2026-04-26 (see `project_3c_bundling_prep` memory for the full plan):
- **Bundler**: uv-standalone with python-build-standalone interpreter (primary); PyInstaller one-folder mode as fallback. PyOxidizer + Nuitka ruled out.
- **Font bundling**: Tauri ships Noto fonts (Regular+Bold only, ~75% size cut on CJK) in resource directory; Chromium consumes via `@font-face` with explicit `unicode-range` descriptors (mandatory — Chromium falls back to fontconfig otherwise, producing tofu on glyphs the bundled Noto contains).
- **fontconfig drop**: replace fc-match with a fontTools-only scanner over the bundled font directory. Two call sites — `loom_core/subs/processing.py:123` (family-name → path for Pillow measurer) and `loom_core/fonts.py:155` (path resolution + silent-substitution detection). ~150–200 lines, self-contained refactor that can land before the Tauri bundling work.
- **Synthesis**: one font directory, three consumers (Python validator, Pillow measurer, Chromium renderer), zero system deps for fonts.

Still open: Tauri auto-updater compat with multi-hundred-MB resource directories; python-build-standalone interpreter pin; unidic-lite 250MB shrinking question.

**Next concrete step (resume here after compact):** Build the fontTools-only font scanner as a **self-contained refactor**, ahead of any Tauri-bundling work. Scope:
1. Rewrite `loom_core/fonts.py` to scan a directory parameter (instead of calling `fc-match`): walk the directory, open each TTF/OTF/TTC with `fontTools.ttLib.TTFont`, read the `name` table (records 1/4/16, prefer Unicode platform encoding) + OS/2 Unicode range bits, build `family_name → (path, ttc_index)` and `path → cmap_dict` maps. Cache build results; lazy-rebuild on directory mtime change.
2. Replace `loom_core/subs/processing.py:123 _resolve_font_path()` to consume the same scanner instead of calling fc-match directly.
3. Add a small test fixture under `tests/fixtures/fonts/` (a handful of TTFs — Latin + one CJK + one Hebrew; pull from Noto OFL for license cleanliness) and rewrite `tests/test_font_validation.py` to point the scanner at the fixture instead of using `@pytest.mark.skipif(not _FC_MATCH_OK)`. Removes Linux-only test asymmetry.
4. Update CI workflow to drop the fontconfig package install step on Linux (no longer needed) and remove the "fontconfig DELIBERATELY ABSENT" comment from the Windows step (the asymmetry no longer exists post-refactor).

Out of scope for this chunk: the actual Tauri resource bundling, the @font-face CSS generation, the Noto-font set selection. Those follow once the scanner can validate any directory we point it at — including, eventually, the Tauri resource dir.
