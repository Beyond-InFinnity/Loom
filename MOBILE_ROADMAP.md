...
- **7a** `packages/player-ui` package skeleton + the seven seam interfaces + move the already-pure modules. Extension consumes them from the package (pure re-export shuffle, zero behavior change, vitest green).
