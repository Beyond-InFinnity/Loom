...
Take any video with a subtitle track in the language you're learning and Loom builds a stacked, reading-friendly display:

```
Sukuna no sōsaku wo suru to nareba—            ← romanization  (macron Hepburn)
        すくな        そうさく                    ← per-character readings (furigana)
宿儺の捜索をするとなれば—                          ← target language (the spoken line)
You will witness terrible scenes…              ← your native language (translation)
```

The ordering follows interlinear-gloss convention used in linguistics — most phonetically accessible at the top, native script in the middle, translation at the bottom — so each reading sits directly above the characters it annotates. It's the same mental model as Duolingo's pinyin display, extended to a full four-layer stack and applied to whatever you're actually watching.
...
Every screenshot below is Loom rendering live over real video — the readings, romanization, and translation are generated by Loom, not the source.
