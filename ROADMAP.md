...
**Recon complete — GO (2026-05-30):** `NETFLIX_RECON.md` — port of the dual-subs overlay from YouTube to Netflix is validated.  Every load-bearing capability confirmed in current (2024–2026) extension source; parse path validated (`spike/netflix/parse-test.mjs`, 24/24).  Key correction vs the original plan: the manifest is MSL-encrypted, so acquisition uses a MAIN-world `JSON.parse`/`JSON.stringify` hook (à la Subadub/Language Reactor), **not** `webRequest` — which makes the discovery path *simpler* than YouTube's (no background observer, no pot-token dance).  One real constraint: image-based (PNG) subtitles on some CJK/Greek/Hebrew titles are OCR-only, detected via missing `webvtt-lssdh-ios8` profile and degraded gracefully.  Per the agreed sequence, the full port lands *after* AMO distribution so it ships as an auto-pushed update.  One owed owner step: run `spike/netflix/capture-kit.js` on a live authenticated session to confirm the real manifest/player/DOM and seed the parser with a real `.vtt`.  See that doc's Findings log.

**Mobile + Loom Player plan of record (2026-07-11, Connor signed off):** `MOBILE_ROADMAP.md` — the local-`.mkv`-with-tap-gloss dream on phones, sequenced as: ① extract the extension's overlay/card/settings stack into a host-agnostic `packages/player-ui` WebView payload behind seven seam interfaces (coupling audit done; seams specced), ② Loom Player desktop (grow Tauri 2 + `tauri-plugin-libmpv`), ③ Android (Kotlin + `dev.jdtech.mpv:libmpv` AAR + transparent WebView; Media3 for MKV sub demux — ffmpeg-kit is retired, don't plan on it), ④ Chromecast custom receiver as an Android output mode (second-screen gloss), ⑤ iOS last. Independent cheap probe: Firefox-Android runs AMO extensions — live-recon YouTube there. Substeps 7a–8c + R1 proposed in that doc.
